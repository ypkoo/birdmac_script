...
for mac in "${macs[@]}"; do
	...
	for period_ in "${periods[@]}"; do
		...
		for check_rate in "${check_rates[@]}"; do
		  for sigma in "${sigmas[@]}"; do
		    for density in "${densities[@]}"; do
					echo "#" $period_-$check_rate-$density >> ../$result_dir/over_degree
					for topology in "${topologies[@]}"; do
						rm -f temp_power
		      	for seed in "${seeds[@]}"; do
							grep "AVG ON" ../$rawdata_dir/$period-$topology-$check_rate-$density-$seed-power >> temp_power
...
for mac in "${macs[@]}"; do
	...
	for period_ in "${periods[@]}"; do
	  ...
	  for sigma in "${sigmas[@]}"; do
	    for density in "${densities[@]}"; do
				for topology in "${topologies[@]}"; do
					echo "#" $period_-$topology-$density >> ../$result_dir/over_rate
					for check_rate in "${check_rates[@]}"; do
						rm -f temp_power
			    	for seed in "${seeds[@]}"; do
							grep "AVG ON" ../$rawdata_dir/$period-$topology-$check_rate-$density-$seed-power >> temp_power
			      done
						awk -v rate="$check_rate" '{sum+=$3}; END {print rate, sum/NR}' temp_power >> ../$result_dir/over_rate
						rm -f temp_power
					done
					echo "" >> ../$result_dir/over_rate
	      done
	    done
	  done
	done
